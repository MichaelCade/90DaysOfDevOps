...
Microsoft deploys multiple regions within a geo-political boundary. 
...
Availability Zones - Provide resiliency between datacenters within a region.  
...
-  Virtual Machines are placed on a virtual network which can provide connectivity to any network. 
...
There are a number of different management portals and consoles we can use to create our resources the preferred route is going to be via JSON templates. 

Idempotent deplyments in incremental or complete mode - i.e repeatable desired state. 

There are a large selection of templaates that can export deployed resource definitions. I like to think about this templating feature to something like AWS CloudFormation or could be Terraform for a multi-cloud option. We will cover Terraform more in the Infrastructure as code section. 
...
This is ideal for update windows so that you can update your images and roll those out with least impact. 
...
Azure also has the Container Registory which provides a private registry for Docker Images, Helm charts, OCI Artifacts and images. More on this again when we reach the containers section. 
...
The goal with serverless is that we only pay for the runtime of the function and not having to have running virtual machines or PaaS applications running all the time. We simply run our function when we need it and then it goes away. 

Azure Functions - Provides serverless code. If we remember back to our first look into public cloud you will remember the abstraction layer of management, with serverless or functions you are only going to be managing the code. 

Event-Driven with massive scale, I have a plan to build something when I get some hands on here hopefully later on. 

Provides input and output binding to manay Azure and 3rd Party Services. 

Supports many different programming languages. (C#, NodeJS, Python, PHP, batch, bash, Golang and Rust. Or any Executable)

Azure Event Grid enables logic to be triggere from services and events. 

Azure Logic App provides a graphical based workflow and integration. 

We can also look at Azure Batch which has the ability to run large-scale jobs on both Windows and Linux nodes with consistent management & scheduling. 
